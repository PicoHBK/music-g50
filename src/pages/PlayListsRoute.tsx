...
import SongsById from "@/components/tempUtils/SongsById";
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
function PlayListsRoute() {
  const { dataPag, hasNextPage, fetchNextPage } = usePlaylists();
        ...
        {dataPag?.pages.map((page) =>
          page.results.map((playlist) => (
            <Dialog key={playlist.id}>
              ...
              >
                <div className="flex flex-col text-myprim-800 bg-myprim-200 px-4 py-2 rounded-lg hover:scale-105 transition select-none">
                  <div className="flex items-center">
                    ...
                    <h4 className="text-myprim-800 font-bold">
                      {playlist.name}
                    </h4>
                  </div>
                </div>
