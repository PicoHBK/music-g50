  ...
  return (
    <div className="flex flex-col w-full">
      <div className="flex">
        ...
        {
          genRes?.map((genre) =>(
            <button key={genre.id} className=" flex justify-center items-center bg-myprim-600 text-myprim-50 px-4 py-2 text-lg rounded-lg hover:scale-105 transition" onClick={() => {setGen(genre);setBorrar(false);setEdit(false)}}>
              <h4>{genre.name}</h4>
            </button>
          ))
        }
        ...
        <div className="flex flex-col w-1/3 px-4 py-2 ">
          ...
          <div className="flex flex-col gap-3 bg-myprim-100 rounded-lg text-myprim-700 px-4 py-2">
            ...
            :
            <form className="flex flex-col w-full gap-2" onSubmit={submitEdit}>
              <input
            name="name"
            type="text"
            value={gen.name}
          ...
          <div className="flex gap-3 justify-center">
            <button type="submit" className="bg-mywarn-400 py-1 px-3 text-mywarn-50 rounded-lg hover:bg-mywarn-300">Guardar</button>
            <button type="button" className="bg-myerror-400 py-1 px-3 text-mywarn-50 rounded-lg hover:bg-myerror-300" onClick={()=>cancelEdit(genRes?.find((genres) => genres.id == gen.id)||null)}>Cancelar</button>
          </div>
            ...
            {borrar&&<>
            <p className="text-myerror-300 font-semibold text-center">Eliminar permanentemente</p>
            <button className="bg-myerror-200 text-myerror-500 rounded-lg py-1 hover:bg-myerror-300" onClick={()=> deleteGenresSubmit(gen.id)}>Si</button>
            </>
            }
            
            {(state.user?.user__id === gen.owner && !edit && !borrar && !loading) && <div className="flex gap-2">
                  <button className="bg-mywarn-500 py-1 px-3 text-mywarn-100 rounded-lg hover:scale-105 transition" onClick={()=> setEdit(!edit)}>Editar</button>
                  <button className="bg-myerror-500 py-1 px-3 text-myerror-100 rounded-lg hover:scale-105 transition"onClick={()=> setBorrar(!borrar)} >Borrar</button>
                </div>}
          </div>
          }
           <section>
      <Dialog>
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Por favor no repita géneros ya existentes</DialogDescription>
          </DialogHeader>
          <GenResForm />
        </DialogContent>
      </Dialog>
      ...
      {gen&&<Songs filterGenRes={gen?.id} filterAlbum={"all"} filterPlayList={"all"}/>}
