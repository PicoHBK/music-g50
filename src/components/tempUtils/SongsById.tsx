...
import { usePlayerContext } from "@/hooks/usePlayerSong";
...
const useSongsByIds = (ids: number[]) => {
  return useQueries({
    ...
    combine: (results) => {
      return {
        songs: results.map((result) => result.data).filter((data): data is SongsType => data !== undefined),
  ...
  return (
    <div>
      <ul className="flex flex-col w-full font-lato">
        {songs.map(song => (
          <li key={song.id} className="flex w-full items-center py-2 border-b border-gray-300 gap-8 hover:bg-myprim-200 p-2 rounded-md">
            <p className="flex-1 text-left font-semibold text-myprim-800">{song.title}</p>
